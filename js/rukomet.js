const handballMatches = [
  { home: "Barcelona Handball", away: "PSG Handball", homeScore: 21, awayScore: 19, live: true },
  { home: "Veszprém", away: "Flensburg", homeScore: 18, awayScore: 18, live: true },
  { home: "THW Kiel", away: "Pick Szeged", homeScore: 25, awayScore: 22, live: false }
];

const containerHandball = document.getElementById("matches");

handballMatches.forEach(match => containerHandball.appendChild(createMatchCard(match)));
...
function animateGoal(match, team) {
  ...
  match.scoreElement.style.background = "yellow";
...
setInterval(() => {
  const liveMatches = handballMatches.filter(m => m.live);
